<script>
    ...
    import { getImageURL, instagramURL } from '$lib/utilities.js'

    let titles;
        inView(titles, (e) => {
            titles = e.target;
            animate(e,{opacity:0},{duration: 1});
        })
</script>

<div class="wrap-xplore">
{#each data.gallery as gallery}
    <h1 bind:this={titles}>{gallery.title}</h1>
{/each}
<div class="wq-main">
    <div class="wq">
    {#each data.gallery as gallery}
        <div class="card">
            <img class="img" src={getImageURL(gallery?.collectionId,gallery?.id, gallery?.submission)} alt="{gallery.title} by {gallery.user}">
            <div class="img-info">
